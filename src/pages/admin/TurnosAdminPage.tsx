  ...
  return (
    <div className="dark:text-zinc-100">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="font-display text-2xl font-light text-ink dark:text-zinc-50">
          Agenda semanal
        </h2>
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="min-w-44 text-center text-sm capitalize text-muted dark:text-zinc-400">
          ...
          {diasSemana.map((dia) => {
            ...
            return (
              <section key={dia.toISOString()}>
                <h3 className="mb-2 border-b border-line pb-2 text-sm capitalize text-muted dark:border-purple-950/40 dark:text-zinc-400">
                  {format(dia, "EEEE d 'de' MMMM", { locale: es })}
                  <span className="ml-2 text-gold dark:text-[#E6C687]">
                    {turnosDelDia.length > 0 && `(${turnosDelDia.length})`}
                  </span>
                </h3>
                {turnosDelDia.length === 0 ? (
                  <p className="py-2 text-xs text-line dark:text-zinc-600">Sin turnos</p>
                ) : (
                  <ul className="space-y-2">
                    {turnosDelDia.map((turno) => (
                      <li
                        key={turno.id}
                        className="flex flex-col gap-3 rounded-card border border-line bg-surface p-4 shadow-soft sm:flex-row sm:items-center sm:justify-between dark:border-purple-950/40 dark:bg-zinc-900 dark:shadow-[0_18px_45px_-35px_rgba(0,0,0,0.9)]"
                      >
                        <div className="flex items-center gap-4">
                          <span className="font-display text-xl text-gold dark:text-[#E6C687]">
                            {format(new Date(turno.fecha_hora), 'HH:mm')}
                          </span>
                          <div>
                            <p className="text-sm text-ink dark:text-zinc-50">
                              {turno.cliente.nombre}
                              <span className="ml-2 text-muted dark:text-zinc-400">
                                {turno.cliente.telefono}
                              </span>
                            </p>
                            <p className="mt-0.5 text-xs text-muted dark:text-zinc-400">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {turno.estado === 'CONFIRMADO' && (
                            <>
                              <Button
                                variant="secondary"
                                className="px-3 py-1.5 text-xs dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300 dark:hover:bg-emerald-500/15"
                              ...
                              <Button
                                variant="danger"
                                className="px-3 py-1.5 text-xs dark:border-rose-500/20 dark:bg-transparent dark:text-rose-400 dark:hover:bg-rose-500/10"
